...
fn main() {
    ...
    let _pool_mem = RayonConfig::memory_intensive()
        .build()
        .expect("memory_intensive");
    println!("   - memory_intensive() -> {} threads", (n / 2).max(1));

    let _pool_lat = RayonConfig::latency_sensitive()
        .build()
        .expect("latency_sensitive");
    println!("   - latency_sensitive() -> {} threads", (n / 2).max(2));

    let phys = num_cpus::get_physical();
    let _pool_phys = RayonConfig::physical_cores_only()
        .build()
        .expect("physical_cores_only");
