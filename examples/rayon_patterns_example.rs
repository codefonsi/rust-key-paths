...
fn main() {
    ...
    println!(
        "small_collection_optimization(small, min_len=500): count = {}",
    ...
    println!(
        "small_collection_optimization(large, min_len=500): count = {}\n",
        count.load(Ordering::Relaxed)
    );
    ...
    println!(
        "efficient_sum(0..10000) = {} (expected {})\n",
        sum, expected
    );
    ...
    println!(
        "reduce_lock_contention(chunk_size=100): {} results",
        strings.len()
    );
