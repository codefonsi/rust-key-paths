...
fn main() {
    ...
    let x_pl_rw: Option<&SomeOtherStruct> =
        SomeComplexStruct::scfs_arc_pl_rwo().get(&instance);
    // Same for Arc<std::sync::Mutex<Option<SomeOtherStruct>>>
    let x_std_m: Option<&SomeOtherStruct> =
        SomeComplexStruct::scfs_arc_std_mo().get(&instance);
    // And Arc<std::sync::RwLock<Option<SomeOtherStruct>>>
    let x_std_rw: Option<&SomeOtherStruct> =
        SomeComplexStruct::scfs_arc_std_rwo().get(&instance);
    // And Arc<parking_lot::Mutex<Option<SomeOtherStruct>>>
    let x_pl_m: Option<&SomeOtherStruct> =
        SomeComplexStruct::scfs_arc_pl_mo().get(&instance);
